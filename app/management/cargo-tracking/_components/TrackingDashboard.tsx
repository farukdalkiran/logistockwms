...
export default function TrackingDashboard({ onNavigate }: DashboardProps) {
  ...
  const COLORS = {
    success: '#03DF95',  // Turkuaz
    returned: '#ef4444', // Kırmızı
    error: '#f97316',    // Turuncu
    today: '#3b82f6',    // Mavi
  };
  ...
  if (loading) {
    return (
      <div className="w-full h-[500px] flex flex-col items-center justify-center gap-6 bg-white rounded-md border border-slate-200 shadow-sm relative overflow-hidden animate-in fade-in duration-300">
    ...
    <div className="w-full flex flex-col gap-6 animate-in fade-in duration-300 font-['Quicksand'] bg-slate-50 p-2 sm:p-4 min-h-screen">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="loop-border shadow-sm flex flex-col justify-center p-5 sm:p-6" style={{ '--loop-color': COLORS.returned } as React.CSSProperties}>
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-black text-red-500 font-mono leading-none">{stats.returned.toLocaleString('tr-TR')}</span>
            <span className="text-[10px] text-red-400 font-bold uppercase tracking-wider">PAKET</span>
          </div>
        </div>
          ...
          <div className="w-full h-80 mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={weeklyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="iade" stroke={COLORS.returned} strokeWidth={3} fillOpacity={1} fill="url(#colorIade)" activeDot={{ r: 5, fill: '#fff', stroke: COLORS.returned, strokeWidth: 3 }} />
                <Area type="monotone" dataKey="hatali" stroke={COLORS.error} strokeWidth={2} fill="none" strokeDasharray="5 5" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-md shadow-sm flex flex-col p-5 sm:p-6 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-2 mt-4 z-10">
            {pieData.map((item, idx) => (
              <div key={idx} className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest bg-slate-50 px-3 py-2 rounded-md border border-slate-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-2">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={weeklyData} margin={{ top: 0, right: 0, left: -25, bottom: 0 }} barSize={25}>
                ...
                <Tooltip 
                  ...
                  cursor={{fill: '#f8fafc'}}
                />
                <Bar dataKey="iade" fill={COLORS.returned} radius={[4, 4, 0, 0]} />
                <Bar dataKey="hatali" fill={COLORS.error} radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-md shadow-sm p-5 sm:p-6 flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-slate-50 border-l-4 border-slate-800 p-4 rounded-md border border-slate-100">
               <span className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Toplam İşlenen Kayıt (Satır)</span>
               <span className="text-2xl font-black text-slate-800 font-mono tracking-tight">{stats.totalRecords.toLocaleString('tr-TR')}</span>
            </div>
            
            <div className="bg-emerald-50 border-l-4 border-[#03DF95] p-4 rounded-md border border-emerald-100">
               <span className="block text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1">Toplam Ürün Miktarı (Kalem)</span>
               <span className="text-2xl font-black text-[#03DF95] font-mono tracking-tight">{stats.totalItems.toLocaleString('tr-TR')}</span>
            </div>

            <div className="bg-slate-900 border border-slate-800 p-4 rounded-md mt-2 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Package className="w-5 h-5 text-[#03DF95]" />
                <div className="flex flex-col">
                  <span className="text-xs font-black text-white uppercase tracking-widest">WMS Veri Motoru</span>
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Cargo_Records DB Aktif</span>
                </div>
              </div>
